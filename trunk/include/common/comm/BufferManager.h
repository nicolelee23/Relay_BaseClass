...
public:
    ...
    */
    ...
    //add for relay
    /*list<Iov> getSendIovList()//m_sendIovList由BufferManager回收
        //注意：基类析构Agent的同时会析构BufferManager对象,所以应该复制到子线程里
    {
        return m_SendIovList;
    }*/

    void  getSendIovListBuffer(string & data)
    {
        list<Iov> ::iterator it;
        for(it = m_SendIovList.begin(); it != m_SendIovList.end(); ++it)
        {
            data.append((char*)(*it).m_Iovec.iov_base);
        }
    }
protected:
